...
from sklearn.decomposition import PCA   #reduce complexity of data by dimentionality reduction
from sklearn.preprocessing import LabelEncoder   #encoding categorical labels into numerical labels
...
lbl = tk.Label(root, text="Heart Disease Detection System", font=('times', 35,' bold '), height=1, width=32,bg="white",fg="red")
...
def Data_Preprocessing():
    ...
    """One Hot Encoding"""   #change categoral data into numeric
...
def Model_Training():
    ...
    #use of support vector machine (binary y/n)
    
    #train model    
    from sklearn.svm import SVC
    svcclassifier = SVC(kernel='linear')    #classify and fit data
    svcclassifier.fit(x_train, y_train)

    y_pred = svcclassifier.predict(x_test)    #prediction
    print(y_pred)
